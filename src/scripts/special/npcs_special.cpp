    ...
    void UpdateAI(const uint32 diff)
    {
        if (checkTimer.Expired(diff))
        {
            ...
            if (!inProgress && time(NULL) > startEvent)
            {
                ...
                MaNGOS::ObjectWorker<Creature, firerespawner> worker(resp);
                ...
                if (Creature* shade = me->SummonCreature(HH_NPC_SHADE, pos.m_positionX, pos.m_positionY, pos.m_positionZ, 0, TEMPSUMMON_MANUAL_DESPAWN, 0))
            ...
            if (inProgress)
            {
                ...
                if (aliveCount ==0 && allCount > 1)
                {
                    ...
                    MaNGOS::AnyPlayerInObjectRangeCheck check(me, 90);
