...
struct npc_hogger_theAI : public ScriptedAI
{
    ...
    void SummonAds()
    {
        ...
        case 1:
        case 3:
            SummonImps(1); // will be replaced by something more epic
            break;
        }
    ...
    void SummonImps(uint8 count)
    {
        for (uint8 i = 0; i < count; i++)
        {
            Position pos;
            m_creature->GetValidPointInAngle(pos, frand(1.0f, 20.0f), frand(0.0f, 2 * M_PI), true);
            if (Creature* imp = m_creature->SummonCreature(NPC_IMP, pos.x, pos.y, pos.z, pos.o, TEMPSUMMON_CORPSE_DESPAWN, 1000))
            {
                summons.Summon(imp);
...
CreatureAI* GetAI_npc_hogger_the(Creature *_Creature)
{
    return new npc_hogger_theAI(_Creature);
}
