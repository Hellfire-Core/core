...
struct npc_magram_spectreAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        if (!UpdateVictim() && checker.Expired(diff))
        {
            checker = 60000;
            GameObject* gob = NULL;
            MaNGOS::NearestGameObjectEntryInObjectRangeCheck check(*m_creature,177746,50.0f);
            MaNGOS::ObjectSearcher<GameObject, MaNGOS::NearestGameObjectEntryInObjectRangeCheck> checker(gob,check);
...
struct npc_gizelton_caravanAI : public ScriptedAI
{
    ...
    void MakeWave(bool north)
    {
        ...
        for (uint8 i = 0; i < 4; i++)
        {
            ...
            if (Creature* enemy = me->SummonCreature(id, pos.m_positionX, pos.m_positionY, pos.m_positionZ, 0, TEMPSUMMON_CORPSE_DESPAWN, 10000))
