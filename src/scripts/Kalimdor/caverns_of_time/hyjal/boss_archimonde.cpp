...
struct mob_doomfire_targettingAI : public NullCreatureAI
{
    ...
    uint32 ChangeWaypointAndTimer()
    {
        ...
        else
        {
            ...
            me->GetMotionMaster()->MovePoint(1, dest.m_positionX, dest.m_positionY, dest.m_positionZ);
            return 6000;
        }
    }
        ...
        if (SummonTimer.Expired(diff))
        {
            ...
            if (pArchimonde && pArchimonde->IsAlive())
            {
                if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == IDLE_MOTION_TYPE)
                {
                    ...
                    me->GetMotionMaster()->MovePoint(2, dest.m_positionX, dest.m_positionY, dest.m_positionZ);
