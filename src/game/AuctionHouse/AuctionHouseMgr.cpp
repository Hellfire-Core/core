...
void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry *auction)
{
    ...
    uint8 biddergmlevel = SEC_PLAYER;
    ...
    if (bidder)
    {
        ...
        biddergmlevel = bidder->GetSession()->GetGMLevel();
    }
    else
    {
        bidderAccId = sObjectMgr.GetPlayerAccountIdByGUID(bidderGuid);
        biddergmlevel = bidderAccId ? AccountMgr::GetGMLevel(bidderAccId) : SEC_PLAYER;
    ...
    {
        if (biddergmlevel >= SEC_GAMEMASTER)
        {
