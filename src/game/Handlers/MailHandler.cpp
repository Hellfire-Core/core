...
void WorldSession::HandleSendMail(WorldPacket & recv_data)
{
    ...
    if (!sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_MAIL) && pl->GetTeam() != rc_team && !HasHigherGMLevel(SEC_GAMEMASTER))
    {
    ...
    if (items_count > 0 || money > 0)
    {
        if (items_count > 0)
        {
            for(uint8 i = 0; i < items_count; ++i)
            {
                ...
                if (HasHigherGMLevel(SEC_GAMEMASTER) && sWorld.getConfig(CONFIG_GM_LOG_TRADE))
                {
    ...
    if (deliver_delay && HasHigherGMLevel(SEC_GAMEMASTER) && sWorld.getConfig(CONFIG_GM_MAIL))
        deliver_delay = 0;
