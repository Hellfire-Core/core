...
void WorldSession::HandleMovementOpcodes(WorldPacket & recv_data)
{
    ...
    if (!MaNGOS::IsValidMapCoord(movementInfo.GetPos()->m_positionX, movementInfo.GetPos()->m_positionY, movementInfo.GetPos()->m_positionZ, movementInfo.GetPos()->o))
        return;
    ...
    if (Player *plMover = mover->ToPlayer())
    {
        ...
        if (mover->HasUnitState(UNIT_STAT_ROOT))
        {
            if (mover->m_movementInfo.pos != movementInfo.pos) // allow rotating in roots
            {
                ...
                movementInfo.pos.m_positionX = mover->m_movementInfo.pos.m_positionX;
                movementInfo.pos.m_positionY = mover->m_movementInfo.pos.m_positionY;
                movementInfo.pos.m_positionZ = mover->m_movementInfo.pos.m_positionX;
                movingGood = false;
            }
        }

        if (sWorld.getConfig(CONFIG_ENABLE_PASSIVE_ANTICHEAT) && !plMover->HasUnitState(UNIT_STAT_LOST_CONTROL | UNIT_STAT_NOT_MOVE) && !plMover->GetSession()->HasHigherGMLevel(SEC_DEVELOPER))
        {
            if (plMover->m_AC_timer == 0 || // time up OR moved long distance and timer is NOT on long interval(caused by teleport)
                (plMover->m_AC_timer < 2500 && (abs(plMover->m_movementInfo.pos.m_positionX - movementInfo.pos.m_positionX) > 15 || abs(plMover->m_movementInfo.pos.m_positionY - movementInfo.pos.m_positionY) > 15)))
            {
        ...
        if (movementInfo.HasMovementFlag(MOVEFLAG_SWIMMING) != plMover->IsInWater())
        {
            ...
            plMover->SetInWater(!plMover->IsInWater() || plMover->GetTerrain()->IsInWater(movementInfo.GetPos()->m_positionX, movementInfo.GetPos()->m_positionY, movementInfo.GetPos()->m_positionZ));
        }
    }
    
    mover->m_movementInfo = movementInfo;
    mover->SetPosition(movementInfo.GetPos()->m_positionX, movementInfo.GetPos()->m_positionY, movementInfo.GetPos()->m_positionZ, movementInfo.GetPos()->o);
    if (mover->GetObjectGuid().IsPlayer())
        mover->ToPlayer()->HandleFallUnderMap(movementInfo.GetPos()->m_positionZ);
