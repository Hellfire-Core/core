    ...
    for (int i=0; i<TRADE_SLOT_TRADED_COUNT; i++)
    {
        ...
        if (traderCanTrade && playerCanTrade)
        {
            ...
            if (myItems[i])
            {
                ...
                if (_player->GetSession()->HasHigherGMLevel(SEC_GAMEMASTER) && sWorld.getConfig(CONFIG_GM_LOG_TRADE))
                {
            ...
            if (hisItems[i])
            {
                ...
                if (_player->pTrader->GetSession()->HasHigherGMLevel(SEC_GAMEMASTER) && sWorld.getConfig(CONFIG_GM_LOG_TRADE))
                {
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    if (_player->pTrader->acceptTrade)
    {
        ...
        {
            if (_player->GetSession()->HasHigherGMLevel(SEC_GAMEMASTER) && _player->tradeGold > 0)
            {
            ...
            if (_player->pTrader->GetSession()->HasHigherGMLevel(SEC_GAMEMASTER) && _player->pTrader->tradeGold > 0)
            {
