...
void WorldSession::HandleWhoOpcode(WorldPacket & recv_data)
{
    ...
    {
        if (!HasHigherGMLevel(SEC_DEVELOPER))
        {
            ...
            if (itr->second->GetSession()->HasHigherGMLevel(SEC_GAMEMASTER) && !gmInWhoList)
                continue;
        }
...
{
    sLog.outDebug("WORLD: Recvd CMSG_LOGOUT_REQUEST Message, security - %lu", GetGMLevel());
    ...
    if (GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING) || GetPlayer()->IsTaxiFlying() ||
        HasHigherGMLevel(sWorld.getConfig(CONFIG_INSTANT_LOGOUT)))
    {
        LogoutPlayer(true);
        return;
    }
...
void WorldSession::HandleAddFriendOpcodeCallBack(QueryResultAutoPtr result, uint32 accountId, std::string friendNote)
{
    ...
    if (result)
    {
        ...
        if (session->HasHigherGMLevel(SEC_DEVELOPER) || sWorld.getConfig(CONFIG_ALLOW_GM_FRIEND) || !AccountMgr::HasHigherGMLevel(friendAcctid, SEC_GAMEMASTER))
            if (friendGuid)
            {
                if (friendGuid==session->GetPlayer()->GetGUID())
                    friendResult = FRIEND_SELF;
                else if (session->GetPlayer()->GetTeam() != team && !sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_ADD_FRIEND) && !session->HasHigherGMLevel(SEC_DEVELOPER))
...
void WorldSession::HandleAddIgnoreOpcodeCallBack(QueryResultAutoPtr result, uint32 accountId)
{
    ...
    if (result)
    {
        ...
        if (IgnoreGuid)
        {
            ...
            if (!tmp || !tmp->GetSession()->HasHigherGMLevel(SEC_DEVELOPER))  // add only players
            {
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& recv_data)
{
    ...
    if (HasHigherGMLevel(SEC_ADMINISTRATOR))
