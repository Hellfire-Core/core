...
{
    Position dest;
    creature.GetRespawnCoord(dest.m_positionX, dest.m_positionY, dest.m_positionZ, &dest.o, &i_wanderDistance);

    if (i_wanderDistance < 0.1f)
        i_wanderDistance = 0.1f;

    const float range = frand(0.1f, i_wanderDistance);
    if (!creature.GetMap()->GetReachableRandomPosition(&creature, dest.m_positionX, dest.m_positionY, dest.m_positionZ, range))
        return;
