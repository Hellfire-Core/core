...
void WorldObject::Relocate(Position pos)
{
    ...
    if(isType(TYPEMASK_UNIT))
        ((Unit*)this)->m_movementInfo.ChangePosition(pos.m_positionX, pos.m_positionY, pos.m_positionZ, pos.o);
}
...
void WorldObject::MonsterSay(int32 textId, uint32 language, uint64 TargetGuid)
{
    ...
    MaNGOS::PacketBroadcaster visitor(*this, &data, 0, range);
...
void WorldObject::MonsterWhisper(int32 textId, uint64 receiver, bool IsBossWhisper)
{
    ...
    char const* text = sObjectMgr.GetMangosStringForDBCLocale(textId);
...
void WorldObject::GetRandomPoint(float x, float y, float z, float distance, float &rand_x, float &rand_y, float &rand_z) const
{
    ...
    rand_x = pos.m_positionX;
    rand_y = pos.m_positionY;
...
void WorldObject::GetValidPointInAngle(Position &pos, float dist, float angle, bool meAsSourcePos, float allowHeightDifference) const
{
    ...
    Position dest;
    dest.m_positionX = pos.m_positionX + (dist+1) * cos(angle);
    dest.m_positionY = pos.m_positionY + (dist+1) * sin(angle);
    // try 2yds more, then backout 2yds for collision prevention

    TerrainInfo const* _map = GetTerrain();
    float ground = _map->GetHeight(dest.m_positionX, dest.m_positionY, MAX_HEIGHT, false);
    ...
    VMAP::VMapFactory::createOrGetVMapManager()->getObjectHitPos(GetMapId(), pos.m_positionX, pos.m_positionY, pos.m_positionZ + 2.0f, dest.m_positionX, dest.m_positionY, dest.m_positionZ + 2.0f, dest.m_positionX, dest.m_positionY, dest.m_positionZ, -1.5f);
    dist = sqrt((pos.m_positionX - dest.m_positionX)*(pos.m_positionX - dest.m_positionX) + (pos.m_positionY - dest.m_positionY)*(pos.m_positionY - dest.m_positionY));
    
    float step = dist / 10.0f;
    for (int j = 0; j < 10; ++j)
    {
        // do not allow too big z changes
        if (fabs(pos.m_positionZ - dest.m_positionZ) > allowHeightDifference)
        {
            dest.m_positionX -= step * cos(angle);
            dest.m_positionY -= step * sin(angle);
            ground = _map->GetHeight(dest.m_positionX, dest.m_positionY, MAX_HEIGHT, false);
            floor = _map->GetHeight(dest.m_positionX, dest.m_positionY, pos.m_positionZ, true);
            dest.m_positionZ = fabs(ground - pos.m_positionZ) <= fabs(floor - pos.m_positionZ) ? ground : floor;
        }
        else
        {
