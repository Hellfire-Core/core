...
namespace VMAP
{
    ...
    bool ModelInstance::intersectRay(const G3D::Ray& pRay, float& pMaxDist, bool pStopAtFirstHit, bool debug) const
    {
        if (!iModel)
        {
#ifdef VMAP_DEBUG
        ...
        bool hit = iModel->IntersectRay(modRay, distance, pStopAtFirstHit,debug);
        if(hit)
        {
            distance *= iScale;
            pMaxDist = distance;
