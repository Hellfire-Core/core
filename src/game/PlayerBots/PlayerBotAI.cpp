...
bool PlayerBotAI::SpawnNewPlayer(WorldSession* sess, uint8 class_, uint32 race_, uint32 mapId, uint32 instanceId, float x, float y, float z, float o, Player* pClone)
{
    ...
    if (!newChar->Create(guid, name, race_, class_, gender, skin, face, hairStyle, hairColor, facialHair, 0))
    {
        sLog.outString("PlayerBotAI::SpawnNewPlayer: Unable to create a player!");
        delete newChar;
        return false;
    }
    newChar->SetMapId(mapId);
    newChar->SetInstanceId(instanceId);
    //newChar->SetAutoInstanceSwitch(false);
    newChar->GetMotionMaster()->Initialize();
    // Set instance
    if (instanceId && mapId > 1) // Not a continent
    {
        InstanceSave *save = sInstanceSaveManager.AddInstanceSave(mapId, instanceId, pClone ? pClone->GetDifficulty() : 0, time(nullptr) + 3600, false, true);
        newChar->BindToInstance(save, true, true);
    }
    // Generate position
    Map* map = sMapMgr.FindMap(mapId, instanceId);
    if (!map)
    {
        sLog.outString("PlayerBotAI::SpawnNewPlayer: Map (%u, %u) not found!", mapId, instanceId);
        delete newChar;
        return false;
    }
    ...
    {
        sLog.outString("PlayerBotAI::SpawnNewPlayer: Unable to add player to map!");
        delete newChar;
        return false;
    }
    sess->SetPlayer(newChar);
    //sess->SetMasterPlayer(mPlayer);
    sObjectAccessor.AddPlayer(newChar);
...
void MageOrgrimmarAttackerAI::UpdateAI(uint32 const diff)
{
    ...
    /// COMBAT AI
    if (me->IsNonMeleeSpellCast(false) || (me->HasAura(AURA_REGEN_MANA) && me->GetPower(POWER_MANA) != me->GetMaxPower(POWER_MANA)))
        return;
    float range = me->IsInCombat() ? 30.0f : frand(15, 30);
    Unit* target = me->SelectNearbyTarget(range);
    ...
    if (me->movespline->Finalized())
    {
        ...
        r = 20;
        if (!me->GetMap()->GetReachableRandomPointOnGround(x, y, z, r))
            return;
    }
    else
    {
...
void PopulateAreaBotAI::BeforeAddToMap(Player* player)
{
    ...
    if (player->GetMapId() != _map || !player->IsWithinDist3d(_x, _y, _z, _radius * 2))
    {
        ...
        while (!map->GetReachableRandomPosition(player, x, y, z, _radius));
        player->Relocate(x, y, z);
        player->SetMapId(_map);
    }
}
