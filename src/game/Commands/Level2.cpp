...
bool ChatHandler::HandleMuteCommand(const char* args)
{
    ...
    if (chr)
    {
        ...
        gmlevel = chr->GetSession()->GetGMLevel();
    }
    else
    {
        account_id = sObjectMgr.GetPlayerAccountIdByGUID(guid);
        gmlevel = AccountMgr::GetGMLevel(account_id);
    }

    if (m_session && (gmlevel >= m_session->GetGMLevel()))
    {
...
bool ChatHandler::HandleKickPlayerCommand(const char *args)
{
    ...
    if (!kickName)
     {
        ...
        if (m_session && (player == m_session->GetPlayer() || player->GetSession()->GetGMLevel() > m_session->GetGMLevel()))
        {
    ...
    else
    {
        ...
        if (m_session && player->GetSession()->GetGMLevel() > m_session->GetGMLevel())
        {
...
bool ChatHandler::HandlePInfoCommand(const char* args)
{
    ...
    if (target && target->GetSession()->HasHigherGMLevel(SEC_GAMEMASTER) &&
        (m_session && !m_session->HasHigherGMLevel(sWorld.getConfig(CONFIG_GM_TRUSTED_LEVEL))))
        return false;
    if (target)
    {
    ...
    QueryResultAutoPtr result = AccountsDatabase.PQuery("SELECT a.username,ap.gmlevel,a.email,a.last_ip,a.last_login "
                                                        "FROM account a "
                                                        "LEFT JOIN account_access ap "
    ...
    if (result)
    {
        ...
        gmlevel = fields[1].GetUInt8();
        if ((gmlevel >= SEC_GAMEMASTER) && m_session && !m_session->HasHigherGMLevel(sWorld.getConfig(CONFIG_GM_TRUSTED_LEVEL)))
            return false;
        ...
        if (!m_session || m_session->HasHigherGMLevel(sWorld.getConfig(CONFIG_GM_TRUSTED_LEVEL)))
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target ? "" : GetMangosString(LANG_OFFLINE)), GetNameLink(name).c_str(), GUID_LOPART(targetGUID), username.c_str(), accId, email.c_str(), gmlevel, last_ip.c_str(), last_login.c_str(), latency);
...
bool ChatHandler::HandleEventAwardCommand(const char* args)
{
    ...
    MaNGOS::AnyPlayerInObjectRangeCheck check(me, 50);
    MaNGOS::ObjectListSearcher<Player, MaNGOS::AnyPlayerInObjectRangeCheck> searcher(targets, check);
...
bool ChatHandler::HandleLookupPlayerIpCommand(const char* args)
{
    ...
    if (sWorld.getConfig(CONFIG_HIDE_GAMEMASTER_ACCOUNTS) && !m_session->HasHigherGMLevel(SEC_GAMEMASTER))
    {
        QueryResultAutoPtr result = AccountsDatabase.PQuery("SELECT account_id, username FROM account WHERE last_ip = '%s' AND account_id NOT IN (SELECT account_id FROM account_access WHERE account_id = account.account_id AND gmlevel >= '3' AND realm_id = '%i')", ip.c_str(), realmID);
...
bool ChatHandler::HandleMmapPathCommand(const char* args)
{
    ...
    {
        wp = player->SummonCreature(WAYPOINT_NPC_ENTRY, pointPath[i].x, pointPath[i].y, pointPath[i].z, 0, TEMPSUMMON_TIMED_DESPAWN, 60);
...
bool ChatHandler::HandleNpcDmginfoCommand(const char* args)
{
    ...
    if (m_session->GetGMLevel() > SEC_PLAYER)
