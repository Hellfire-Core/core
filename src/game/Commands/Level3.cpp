...
bool ChatHandler::HandleReloadMangosStringCommand(const char*)
{
    ...
    SendGlobalGMSysMessage("DB table `mangos_string` reloaded.");
    return true;
}
...
bool ChatHandler::HandleAccountSetGMLevelCommand(const char* args)
{
    ...
    if (getSelectedPlayer() && arg1 && !arg2)
    {
        ...
        if ((gm > SEC_CONSOLE) || (gm < SEC_PLAYER))
        {
        ...
        targetPermissions = targetPlayer->GetSession()->GetGMLevel();
        if (targetPermissions >= m_session->GetGMLevel() || gm >= m_session->GetGMLevel())
        {
        ...
        AccountsDatabase.PExecute("UPDATE account_access SET gmlevel = '%u' WHERE account_id = '%u' AND realm_id = '%u'", gm, targetAccountId, realmID);
        return true;
    }
    else
    {
        ...
        uint32 plSecurity = m_session ? m_session->GetGMLevel() : SEC_CONSOLE;

        /// can set security level only for target with less security and to less security that we have
        /// This is also reject self apply in fact
        targetPermissions = AccountMgr::GetGMLevel(targetAccountId);
...
bool ChatHandler::HandleAccountSetPasswordCommand(const char* args)
{
    ...
    uint32 targetPermissions = AccountMgr::GetGMLevel(targetAccountId);

    /// m_session==NULL only for console
    uint32 plPermiossions = m_session ? m_session->GetGMLevel() : SEC_CONSOLE;
...
bool ChatHandler::HandleUnBanHelper(BanMode mode, const char* args)
{
    ...
    if (m_session && m_session->GetGMLevel() <= SEC_TICKETMASTER && mode == BAN_ACCOUNT || mode == BAN_CHARACTER)
    {
        uint32 account = 0;
        if (mode == BAN_ACCOUNT)
        ...
        if (result)
        {
            ...
            if (uint32 punished_by_acc = sObjectMgr.GetPlayerAccountIdByPlayerName(punished_by))
                if (AccountMgr::GetGMLevel(punished_by_acc) > m_session->GetGMLevel())
                {
                    m_session->SendNotification(LANG_YOU_NOT_HAVE_PERMISSION);
                    return false;
                }
...
bool ChatHandler::HandleBanInfoIPCommand(const char* args)
{
    ...
    PSendSysMessage(LANG_BANINFO_IPENTRY,
        ...
        permanent ? GetMangosString(LANG_BANINFO_INFINITE):secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetString(), fields[5].GetString(), fields[6].GetString());
    return true;
}
...
bool ChatHandler::HandlePLimitCommand(const char *args)
{
    if (*args)
    {
        ...
        if (!sWorld.GetMinimumGMLevel() >= SEC_PLAYER)
            sWorld.KickAllWithoutPermissions(sWorld.GetMinimumGMLevel());
    }

    uint32 pLimit = sWorld.GetPlayerAmountLimit();
    uint8 requiredgmlevel = sWorld.GetMinimumGMLevel();

    PSendSysMessage("Player limits: amount %u, required gmlevel %lu.", pLimit, requiredgmlevel);
...
bool ChatHandler::HandleGMListFullCommand(const char* /*args*/)
{
    ...
    QueryResultAutoPtr result = AccountsDatabase.PQuery("SELECT username, gmlevel FROM account JOIN account_access WHERE gmlevel >= '%u' AND realm_id = '%u'", SEC_GAMEMASTER, realmID);
    if (result)
    {
...
bool ChatHandler::HandleFreezeCommand(const char *args)
{
    ...
    if (player == m_session->GetPlayer() || player->GetSession()->GetGMLevel() > m_session->GetGMLevel())
    {
...
{
    sWorld.KickAllWithoutPermissions(m_session ? SEC_ADMINISTRATOR : 0);
    return true;
}
...
bool ChatHandler::HandleCharacterPremadeGearCommand(const char* args)
{
...
bool ChatHandler::HandleCharacterPremadeSaveGearCommand(const char* args)
{
    ...
    uint32 entry = 0;
    QueryResultAutoPtr result(GameDataDatabase.Query("SELECT MAX(`entry`) FROM `player_premade_item_template`"));
    if (result)
    {
        Field* fields = result->Fetch();
    ...
    if (!GameDataDatabase.PExecute("INSERT INTO `player_premade_item_template` (`entry`, `class`, `level`, `name`) VALUES (%u, %u, %u, '%s')", entry, pPlayer->GetClass(), pPlayer->GetLevel(), templateName.c_str()))
        return false;
    ...
    for (int i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; ++i)
    {
        ...
        {
            GameDataDatabase.DirectPExecute("INSERT INTO `player_premade_item` (`entry`, `item`, `enchant`) VALUES (%u, %u, %u)", entry, pItem->GetEntry(), pItem->GetEnchantmentId(PERM_ENCHANTMENT_SLOT));
        }
    }
...
bool ChatHandler::HandleCharacterPremadeSpecCommand(const char* args)
{
    Player* pPlayer = getSelectedPlayer();
    if (!pPlayer)
    {
...
bool ChatHandler::HandleCharacterPremadeSaveSpecCommand(const char* args)
{
    Player* pPlayer = m_session->GetPlayer();
    ...
    uint32 entry = 0;
    QueryResultAutoPtr result(GameDataDatabase.Query("SELECT MAX(`entry`) FROM `player_premade_spell_template`"));
    if (result)
    {
        Field* fields = result->Fetch();
        entry = fields[0].GetUInt32() + 1;
    ...
    templateName = EscapeString(templateName);
    if (!GameDataDatabase.PExecute("INSERT INTO `player_premade_spell_template` (`entry`, `class`, `level`, `name`) VALUES (%u, %u, %u, '%s')", entry, pPlayer->GetClass(), pPlayer->GetLevel(), templateName.c_str()))
        return false;
    ...
    result = RealmDataDatabase.PQuery("SELECT DISTINCT `spell` FROM `character_spell` WHERE `disabled`=0 && `active`=1 && `guid`=%u", pPlayer->GetGUIDLow());
    ...
    if (result)
    {
        do
        {
            ...
            bool skip = false;;
            for (auto const& itr : pInfo->spell)
            {
            ...
            GameDataDatabase.DirectPExecute("INSERT INTO `player_premade_spell` (`entry`, `spell`) VALUES (%u, %u)", entry, spellId);
        } while (result->NextRow());
    }
