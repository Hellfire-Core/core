...
bool ChatHandler::HandleNameAnnounceCommand(const char* args)
{
    ...
    uint32 textid;
    if (m_session->HasHigherGMLevel(SEC_ADMINISTRATOR))
        textid = LANG_ANNOUNCE_COLOR_ADM;
    else if (m_session->HasHigherGMLevel(SEC_DEVELOPER))
        textid = LANG_ANNOUNCE_COLOR_HDEV;
    else if (m_session->HasHigherGMLevel(SEC_BASIC_ADMIN))
        textid = LANG_ANNOUNCE_COLOR_HGM;
    else if (m_session->HasHigherGMLevel(SEC_GAMEMASTER | SEC_TICKETMASTER))
        textid = LANG_ANNOUNCE_COLOR_GM;
    else if (m_session->HasHigherGMLevel(SEC_MODERATOR))
        textid = LANG_ANNOUNCE_COLOR_MODERATOR;
    else
        return false;
...
bool ChatHandler::HandleGMTicketAssignToCommand(const char* args)
{
    ...
    if (!tarGUID || !AccountMgr::HasHigherGMLevel(accid, SEC_GAMEMASTER))
    {
...
bool ChatHandler::HandleGMTicketUnAssignCommand(const char* args)
{
    ...
    if (plr && plr->IsInWorld() && m_session && plr->GetSession()->GetGMLevel() > m_session->GetGMLevel())
    {
