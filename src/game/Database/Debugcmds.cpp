        ...
        if (Map* map = m_session->GetPlayer()->GetMap())
        {
            ...
            for (InstanceMap::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
            {
                if (Player* i_pl = i->getSource())
                    ...
                    {
                        max++;
                        if (range == 0 || i_pl->GetDistance2d(homepos.m_positionX, homepos.m_positionY) < range)
                        {
...
bool ChatHandler::HandleDebugThreatList(const char * /*args*/)
{
    ...
    uint32 max_count = 0;
    if (!m_session->HasHigherGMLevel(SEC_DEVELOPER))
    {
...
bool ChatHandler::HandleDebugCellCommand(const char* args)
{
    ...
    Map* map = m_session->GetPlayer()->GetMap();
    CellPair pair = MaNGOS::ComputeCellPair(m_session->GetPlayer()->GetPositionX(), m_session->GetPlayer()->GetPositionY()).normalize();
    ...
    MaNGOS::UnitListSearcher<checker> searcher(unitlist, Check);
    TypeContainerVisitor<MaNGOS::UnitListSearcher<checker>, GridTypeMapContainer> visitor(searcher);
    Cell cell(pair);
    cell.SetNoCreate();
