...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    uint8 minimumGMLevel = sWorld.GetMinimumGMLevel();
    sLog.outDebug("Allowed gmlevel: %lu Player gmlevel: %lu", minimumGMLevel, gmlevel);
    if (!(gmlevel >= minimumGMLevel))
    {
        ...
        sLog.outDetail("WorldSocket::HandleAuthSession: User tries to login without enough gmlevel");
        return -1;
    }
    ...
    ACE_NEW_RETURN(m_Session, WorldSession(id, this, gmlevel, expansion, locale, mutetime, mutereason, trollmutetime, trollmutereason, accFlags, opcDis), -1);
