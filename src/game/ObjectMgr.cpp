...
{
    {
        ...
        //                                                               0        1        2        3       4
        QueryResultAutoPtr result(GameDataDatabase.Query("SELECT `entry`, `class`, `level`, `role`, `name` FROM `player_premade_item_template`"));
        ...
        do
        {
            ...
            switch (requiredClass)
            {
                ...
                case CLASS_DRUID:
                    break;
                default:
                    sLog.outBasic("Error: Wrong class %hhu for entry %u in table `player_premade_template`", requiredClass, entry);
                    continue;
            }

            if (!(level >= 1 && level <= MAX_LEVEL))
            {
                sLog.outBasic("Error: Wrong level %hhu for entry %u in table `player_premade_template`", level, entry);
                continue;
            }
...

    {
        ...
        //                                                               0        1       2          3
        QueryResultAutoPtr result(GameDataDatabase.Query("SELECT `entry`, `item`, `enchant`, `team` FROM `player_premade_item`"));
        ...
        do
        {
            ...
            {
                sLog.outBasic("Error: Wrong entry %u in table `player_premade_item`", entry);
                continue;
            }
            ...
            switch (team)
            {
                ...
                case ALLIANCE:
                    break;
                default:
                    sLog.outBasic("Error: Wrong team %u for entry %u in table `player_premade_item`", team, entry);
                    continue;
            }
...

    {
        ...
        //                                                               0        1        2        3       4
        QueryResultAutoPtr result(GameDataDatabase.Query("SELECT `entry`, `class`, `level`, `role`, `name` FROM `player_premade_spell_template`"));
...

    {
        ...
        //                                                               0        1
        QueryResultAutoPtr result(GameDataDatabase.Query("SELECT `entry`, `spell` FROM `player_premade_spell`"));
        ...
        do
        {
            ...
            {
                sLog.outBasic("Error: Wrong entry %u in table `player_premade_spell`", entry);
                continue;
            }
...
void ObjectMgr::ApplyPremadeGearTemplateToPlayer(uint32 entry, Player* pPlayer) const
{
    ...
    if (itr == m_playerPremadeGearMap.end())
    {
        sLog.outBasic("Error: Attempt to apply non-existent premade template to player (%u)", entry);
        return;
    }

    if (pPlayer->GetClass() != itr->second.requiredClass)
    {
        sLog.outBasic("Error: Attempt to apply premade template (%u) to a player with wrong class", entry);
        return;
    }
    ...
    if (!itr->second.spells.empty())
    {
        ...
        for (auto spellId : itr->second.spells)
        {
            ...
            if (firstRankId && firstRankId != spellId && GetTalentSpellPos(firstRankId))
                pPlayer->LearnSpell(firstRankId);
