...
bool ChatHandler::HandleAccountDeleteCommand(const char* args)
{
    ...
    if(m_session)
    {
        ...
        if (AccountMgr::GetGMLevel(account_id) >= m_session->GetGMLevel())
        {
...
bool ChatHandler::HandleAccountOnlineListCommand(const char* args)
{
    ...
    do
    {
        ...
        //                                                                  0         1          2              3
        QueryResultAutoPtr resultLogin = AccountsDatabase.PQuery("SELECT username, last_ip, gmlevel, expansion_id "
