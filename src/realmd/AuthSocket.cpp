...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    //                                          0               1                   2            3         4        5        6
    result = AccountsDatabase.PQuery("SELECT pass_hash, account.account_id, account_state_id, token_key, last_ip, gmlevel, email "
                                     "FROM account JOIN account_access ON account.account_id = account_access.account_id "
    ...
    QueryResultAutoPtr  emailbanresult = AccountsDatabase.PQuery("SELECT email FROM email_banned WHERE email = '%s'", (*result)[6].GetString());
    if (emailbanresult)
    {
        pkt << uint8(WOW_FAIL_BANNED);
    ...
    accountgmlevel = fields[5].GetUInt8();
    ...
    switch (_build)
    {
        ...
        case 6005:                                          // 1.12.2
        {
            ...
            for (RealmList::RealmMap::const_iterator  i = sRealmList.begin(); i != sRealmList.end(); ++i)
            {
                ...
                if (!ok_build || !(accountgmlevel >= i->second.requiredGMLevel))
        ...
        default:                                            // and later
        {
            ...
            for (RealmList::RealmMap::const_iterator  i = sRealmList.begin(); i != sRealmList.end(); ++i)
            {
                ...
                uint8 lock = (accountgmlevel >= i->second.requiredGMLevel) ? 0 : 1;
                ...
                // Show offline state for lower gmlevel because it seems still able to login even if it is locked
                if (!(accountgmlevel >= i->second.requiredGMLevel))
                    realmFlags = RealmFlags(realmFlags | REALM_FLAG_OFFLINE);
